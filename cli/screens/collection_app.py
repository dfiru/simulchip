...
class BaseScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield RichLog(
            id="log-panel", classes="log-panel", wrap=True, highlight=True, markup=True
        )
...
class FilterableDataTable(Widget):
    ...
    class FilterChanged(Message):
        ...
        def __init__(self, filter_text: str) -> None:
            """Initialize FilterChanged message with filter text."""
            self.filter_text = filter_text
            super().__init__()

    def __init__(
        self, placeholder: str = "Type to filter...", table_id: str = "data-table"
    ):
        """Initialize the FilterableDataTable widget.

        Args:
            placeholder: Placeholder text for the filter input.
            table_id: ID for the data table widget.
        """
        super().__init__()
...
class PacksScreen(BaseScreen):
    ...
    def __init__(self, collection_manager: CollectionManager, api: NetrunnerDBAPI):
        ...
        Args:
            collection_manager: Manager for the card collection.
    ...
    def compose(self) -> ComposeResult:
        ...
        self.filterable_table = FilterableDataTable(
            placeholder="Type to filter packs...", table_id="pack-table"
        )
        self.filterable_table.data_table.add_columns(
            "Status", "Code", "Name", "Cycle", "Date"
        )
    ...
    async def populate_table(self) -> None:
        ...
        if self.show_mine_only:
            ...
            filtered_packs = [
                p for p in all_packs if self.collection_manager.has_pack(p["code"])
            ]
        else:
        ...
        if self.filter_text:
            ...
            filtered_packs = [
                ...
                or search_lower in p.get("cycle", "").lower()
            ]

        # Sort by date (newest first)
        filtered_packs.sort(
            key=lambda p: p.get("date_release", "") or "0000", reverse=True
        )
    ...
    async def on_filterable_data_table_filter_changed(
        self, message: FilterableDataTable.FilterChanged
    ) -> None:
...
class CardsScreen(BaseScreen):
    ...
    def compose(self) -> ComposeResult:
        ...
        # Use the FilterableDataTable widget
        self.filterable_table = FilterableDataTable(
            placeholder="Type to filter cards...", table_id="card-table"
        )
        self.filterable_table.data_table.add_columns(
            "Count", "Card", "Type", "Faction", "Pack"
        )
        yield self.filterable_table
    ...
    async def populate_card_table(self) -> None:
        ...
        if self.filter_text:
            ...
            filtered_cards = [
                ...
                or search_lower in c.get("type_code", "").lower()
                or search_lower in c.get("faction_code", "").lower()
                or search_lower in c.get("pack_code", "").lower()
            ]
    ...
    def action_increment_card(self) -> None:
        ...
        if table.cursor_row is not None and table.cursor_row < len(self.card_codes):
            ...
            logger.warning(
                f"Cannot increment: cursor_row={table.cursor_row}, card_codes_len={len(self.card_codes)}"
            )
    ...
    def action_decrement_card(self) -> None:
        ...
        if table.cursor_row is not None and table.cursor_row < len(self.card_codes):
            ...
            logger.warning(
                f"Cannot decrement: cursor_row={table.cursor_row}, card_codes_len={len(self.card_codes)}"
            )
...
class StatsScreen(BaseScreen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(
            "Loading statistics...", id="stats-summary", classes="stats-summary"
        )
        ...
        with Horizontal(classes="button-bar"):
            yield Button(
                "⚠️ Re-initialize Collection", id="reinit-button", variant="error"
            )

        yield Static("nav: (w)elcome (p)acks (c)ards • q: quit", classes="status-bar")
        yield Footer()
        yield from super().compose()  # Add log panel
    ...
    async def update_stats(self) -> None:
        ...
        for pack_code in sorted(self.collection_manager.owned_packs):
            ...
            table.add_row(pack_name, str(expected), str(actual), complete_display)
    ...
    def action_reinitialize(self) -> None:
        ...
        self.notify(
            "⚠️  Press 'y' to confirm PERMANENT DELETION of collection, or any other key to cancel",
            timeout=10,
        )
    ...
    async def on_key(self, event) -> None:
        ...
        if (
            hasattr(self, "_awaiting_reinit_confirmation")
            and self._awaiting_reinit_confirmation
        ):
            ...
            if event.key.lower() == "y":
                try:
                    ...
                    # Log success
                    logger.info(
                        f"Collection re-initialized: {len(result['removed'])} items reset"
                    )
                    ...
                    self.notify(
                        f"❌ Error during re-initialization: {e}",
                        severity="error",
                        timeout=10,
                    )
            else:
...
class CollectionMainApp(App[str]):
    ...
    def __init__(self, collection_manager, api, collection_file=None):
        """Initialize the CollectionApp.

        Args:
            collection_manager: Manager for the card collection.
            api: NetrunnerDB API instance.
            collection_file: Optional path to collection file.
        """
        super().__init__()
        self.collection_manager = collection_manager
        self.api = api
    ...
    def on_mount(self) -> None:
        ...
        self.install_screen(
            PacksScreen(self.collection_manager, self.api), name="packs"
        )
        self.install_screen(
            CardsScreen(self.collection_manager, self.api), name="cards"
        )
        self.install_screen(
            StatsScreen(self.collection_manager, self.api), name="stats"
        )
